            ...
            <form method="POST" action="{{ route('admin.units.update', $unit) }}" class="space-y-5">
                ...
                <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <input type="text" id="no_rangka" name="no_rangka" value="{{ old('no_rangka', $unit->no_rangka) }}"
                    ...
                    <div>
                        ...
                        <input type="text" id="tipe_motor" name="tipe_motor"
                            value="{{ old('tipe_motor', $unit->tipe_motor) }}" class="h-11 w-full rounded-lg border bg-transparent px-4 py-2.5 text-sm text-gray-800 focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90
                    ...
                    <div>
                        ...
                        <input type="text" id="warna" name="warna" value="{{ old('warna', $unit->warna) }}" class="h-11 w-full rounded-lg border bg-transparent px-4 py-2.5 text-sm text-gray-800 focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90
                    ...
                    <div>
                        ...
                        <input type="number" id="tahun" name="tahun" value="{{ old('tahun', $unit->tahun) }}" min="2000"
                            max="{{ date('Y') }}"
                    ...
                    <div>
                        ...
                        <input type="number" id="harga" name="harga" value="{{ old('harga', $unit->harga) }}" min="0"
                            class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90" />
                        @error('harga')
                            <p class="mt-1.5 text-xs text-error-500">{{ $message }}</p>
                        @enderror
                            ...
                            <div class="sm:col-span-2">
                                ...
                                @if ($unit->foto)
                                    <div class="mb-3 flex items-start gap-4">
                                        <img src="{{ $unit->foto_url }}" alt="Foto Unit"
                                            class="w-32 h-24 object-cover rounded-lg border border-gray-200 dark:border-gray-700" />
                                        <div class="space-y-2">
                                            <p class="text-xs text-gray-400">Foto saat ini</p>
                                            <label class="flex items-center gap-2 cursor-pointer">
                                                <input type="checkbox" name="hapus_foto" value="1"
                                                    class="w-4 h-4 rounded border-gray-300 text-error-500 focus:ring-error-500" />
                                                <span class="text-xs text-error-600 dark:text-error-400">Hapus foto ini</span>
                                            </label>
                                        </div>
                                    </div>
                                @endif
