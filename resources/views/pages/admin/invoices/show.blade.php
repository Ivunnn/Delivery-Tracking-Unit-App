        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            @if ($invoice->status_bayar === 'belum_bayar')
                <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-gray-900">
                    ...
                    <form method="POST"
                        action="{{ route('admin.invoices.update-biaya', $invoice) }}"
                        class="flex items-end gap-3">
                        @csrf
                        <div class="flex-1">
                            <label for="biaya_pengiriman" class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                                Biaya Pengiriman (Rp)
                            </label>
                            <input type="number" id="biaya_pengiriman" name="biaya_pengiriman"
                                value="{{ old('biaya_pengiriman', $invoice->biaya_pengiriman) }}"
                                min="0" placeholder="0"
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-gray-900">
                ...
                <dl class="space-y-2">
                    <div>
                        ...
                        <dd class="text-sm font-medium text-gray-800 dark:text-white/90">{{ $invoice->order->customer->name }}</dd>
                    </div>
                    @if ($invoice->order->customer->nama_toko)
                        <div>
                            <dt class="text-xs text-gray-400 mb-0.5">Nama Toko</dt>
                            <dd class="text-sm text-gray-800 dark:text-white/90">{{ $invoice->order->customer->nama_toko }}</dd>
                        </div>
                    @endif
                    <div>
                    ...
                    <div>
                        ...
                        <dd class="text-sm text-gray-800 dark:text-white/90">{{ $invoice->order->customer->phone ?? '-' }}</dd>
                    </div>
                </dl>
            </div>
