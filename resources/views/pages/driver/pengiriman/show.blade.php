...
<div class="space-y-6">
    ...
    @if ($pengiriman->status !== 'selesai')
        <div x-data="{
            ...
            <form method="POST"
                action="{{ route('driver.pengiriman.update-status', $pengiriman) }}"
                class="space-y-4">
                @csrf
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-2 sm:grid-cols-4">
                        @php
                            $statusOptions = [
                                'berangkat'        => ['label' => 'Berangkat',        'color' => 'blue'],
                                'dalam_perjalanan' => ['label' => 'Dalam Perjalanan', 'color' => 'yellow'],
                                'tiba'             => ['label' => 'Tiba di Lokasi',   'color' => 'green'],
                                'selesai'          => ['label' => 'Selesai',          'color' => 'gray'],
                            ];
                            $currentIndex = array_search($pengiriman->status, array_keys($statusOptions));
                        @endphp
                        @foreach ($statusOptions as $val => $opt)
                            @php $idx = array_search($val, array_keys($statusOptions)); @endphp
                            <label class="flex items-center gap-2 rounded-lg border p-3 cursor-pointer transition
                                {{ $idx <= $currentIndex ? 'opacity-40 cursor-not-allowed' : 'hover:border-brand-300' }}
                                {{ $errors->has('status') ? 'border-error-300' : 'border-gray-200 dark:border-gray-700' }}">
                                <input type="radio" name="status" value="{{ $val }}"
                                    {{ $idx <= $currentIndex ? 'disabled' : '' }}
                ...
                <div>
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <span x-show="lat" class="text-xs text-success-500" x-text="`✓ ${lat?.toFixed(5)}, ${lng?.toFixed(5)}`"></span>
    ...
    @if (in_array($pengiriman->status, ['tiba', 'selesai']))
        <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-gray-900">
            ...
            <form method="POST"
                action="{{ route('driver.pengiriman.upload-bukti', $pengiriman) }}"
